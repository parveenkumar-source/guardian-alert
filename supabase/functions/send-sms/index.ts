  ...
  try {
    ...
    const results = await Promise.allSettled(
      contacts.map(async (contact) => {
        ...
        if (!res.ok) {
          console.error(`Twilio failed for ${contact.phone}:`, data);
          return { contact: contact.name, phone: contact.phone, success: false, error: data.message || "Twilio error" };
        }
        return { contact: contact.name, phone: contact.phone, success: true, sid: data.sid };
      })
    );
